  ...
  if (process.env.RESEND_API_KEY && process.env.RESEND_FROM_EMAIL && customerEmail) {
    try {
      await sendEmail({
        ...
        html: `
          <div style="font-family:Arial,sans-serif;max-width:620px;margin:0 auto;color:#111;line-height:1.6">
            <h1 style="font-size:30px;margin-bottom:8px">Your Interval order is in.</h1>
            <p>Hi ${escapeHtml(customerName)},</p>
            <p>Thanks for ordering <strong>Issue 01 of The Interval</strong>.</p>
            <div style="border:2px solid #111;padding:18px;margin:24px 0">
              <p style="margin:0"><strong>Order:</strong> Issue 01 · September 2026</p>
              <p style="margin:6px 0"><strong>Amount:</strong> ${orderAmount}</p>
              <p style="margin:6px 0"><strong>Shipping:</strong> Included</p>
              <p style="margin:6px 0"><strong>Payment ID:</strong> ${escapeHtml(razorpay_payment_id)}</p>
              <p style="margin:6px 0"><strong>Delivery to:</strong> ${escapeHtml(deliveryAddress)}</p>
            </div>
            <p>We'll dispatch your copy from Mumbai within <strong>3 working days</strong>.</p>
      ...
      await sendEmail({
        ...
        html: `
          <div style="font-family:Arial,sans-serif;max-width:700px;color:#111;line-height:1.6">
            <h1 style="font-size:26px">New Interval order</h1>
            <p><strong>Issue 01 · September 2026</strong></p>
            <p><strong>Amount:</strong> ${orderAmount}<br>
            <strong>Payment ID:</strong> ${escapeHtml(razorpay_payment_id)}<br>
            <strong>Razorpay Order ID:</strong> ${escapeHtml(razorpay_order_id)}</p>
            <hr>
            <h2>Customer</h2>
            <p><strong>Name:</strong> ${escapeHtml(customerName)}<br>
            <strong>Email:</strong> ${escapeHtml(customerEmail)}<br>
            <strong>Phone:</strong> ${escapeHtml(phone)}<br>
            <strong>Address:</strong> ${escapeHtml(deliveryAddress)}</p>
            <p><a href="https://dashboard.razorpay.com/app/orders/${encodeURIComponent(razorpay_order_id)}">Open order in Razorpay</a></p>
          </div>
        `,
