...
module.exports=async function(req,res){
 ...
 try{
  const o=(await readOrders()).find(x=>x.id===id);if(!o)return res.status(404).json({error:"Manual order not found."});
  const name=escapeHtml(o.name),product=escapeHtml(o.product||"The Interval"),tracking=escapeHtml(o.tracking||"");
  await sendEmail({to:o.email,subject:"A little Interval is on its way",
 ...
 }catch(e){console.error("Gift email error:",e);return res.status(500).json({error:"Could not send the gift email."})}
};